...
def eotp_train(
    ...
    while not end_flag.value:
        ...
        #fix bug
        if fix_cnt and (rank==2 or rank==10):
            print('fix for agent '+str(rank))
            for i in range(len(meta_gradient)):
                if meta_gradient[i] is not None:
                    meta_gradient[i].zero_()
            fix_cnt = False
        ...
        try:
            barr2.wait(timeout=20)
        except RuntimeError:
            print('Barrier 2 break')
            pass
